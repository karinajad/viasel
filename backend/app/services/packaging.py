...
def add_package_quote(
    ...
    *,
    ...
    """Take a vendor's bid for the whole lot, priced per unit."""
    if pkg.state != OPEN:
...
def _read(session: Session, pkg: SourcingPackage, lines: Sequence[DemandLine], quotes: Sequence[Quote]) -> PackageRead:
    ...
    return PackageRead(
        ...
        awarded_extended=(
            round(_f(selected.unit_price) * total_qty, 2) if selected else None
        ),
    )
...
def leveling(
    ...
    """Bid leveling: every bid on the same footing — per denominator unit, and vs. the ROM.

    Raw unit prices don't compare across vendors until they're normalized to the natural
    denominator, and they don't mean anything until they're set against what the executed
    record says the lot should cost.
    """
    if not quotes:
        return []
    total_qty = sum(dl.qty for dl in lines)
    ...
    low = min(_f(q.unit_price) for q in quotes)

    rows = []
    for q in quotes:
        unit = _f(q.unit_price)
        rows.append(
            LevelingRow(
                quote_id=q.id,
                vendor=q.vendor,
