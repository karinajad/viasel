...
import type { DemandLineRow, EquipmentType, RomBand, RomPriceRequest } from './types/rom'
...
const TIER_COLOR: Record<string, string> = {
  high: '#2f6f4f', medium: '#b7791f', low: '#b23a3a', none: '#6b7280',
}
...
function App() {
  ...
  const typesQ = useQuery({
    ...
    queryFn: () => apiGet<EquipmentType[]>('/equipment-types'),
  })
  const types = typesQ.data ?? []
  const unitCodes = [...new Set(types.map((t) => t.unit_type_code))].sort()
  const subsFor = (code: string) =>
  ...
  const saveM = useMutation({
    mutationFn: () =>
      apiPost<DemandLineRow>('/demand-lines', {
        project_id: PROJECT,
        qty: Number(qty),
        equipment_type_id: typeRow?.id ?? null,
        spec_attributes: { type_query: type, denominator, size, sub: effectiveSub },
        rom_unit_price: band?.unit_mid ?? null,
        rom_confidence: band?.confidence_tier ?? null,
  ...
  return (
    <main style={{ fontFamily: 'system-ui, sans-serif', color: '#1a1a1a', maxWidth: 900, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
      <h1 style={{ marginBottom: 2 }}>Viasel — Design</h1>
      <p style={{ color: '#6b7280', marginTop: 0 }}>
        Price a requirement from executed history, save it as demand, freeze it.
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginTop: 14 }}>
        <div style={card}>
          ...
          <button onClick={price} disabled={priceM.isPending}
        ...
        <div style={card}>
          ...
          {band && (
            <>
              ...
              <button onClick={() => saveM.mutate()} disabled={saveM.isPending || band.unit_mid == null}
                style={{ marginTop: 14, width: '100%', padding: '9px', background: '#fff', color: '#1a1a1a', border: '1px solid #2f6f4f', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>
...
function DemandBoard() {
  ...
  const q = useQuery({
    queryKey: ['demand-lines', PROJECT],
    queryFn: () => apiGet<DemandLineRow[]>(`/demand-lines?project=${PROJECT}`),
  })
  const lines = q.data ?? []

  const freezeM = useMutation({
    mutationFn: () =>
      apiPost('/freeze', { line_ids: selected, project_id: PROJECT, scope, actor: 'web' }),
    ...
    <div style={{ ...card, marginTop: 18 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={() => freezeM.mutate()} disabled={selected.length === 0 || freezeM.isPending}
            style={{ padding: '6px 14px', background: selected.length ? '#2f6f4f' : '#c9ccd1', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, cursor: selected.length ? 'pointer' : 'default' }}>
      ...
      {lines.length > 0 && (
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginTop: 8 }}>
          ...
          <tbody>
            {lines.map((d) => (
              <tr key={d.id} style={{ borderTop: '1px solid #eef0f3' }}>
